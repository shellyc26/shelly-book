...
Leslie gets out the codebook to look at how the variables of interest were coded. Much of the research seemed to be with younger adults who were in college or who were recent college grads, which could be a unique group given their high level of education and the existence of cell phones for a large part of their young lives. With this in mind, the three decide to examine five variables: `USETECH`, `HAPPY`, `SEX`, `AGE`, and `DEGREE.` Leslie uses `summary(gss.2018)` to examine the variables: 
...
Leslie and Kiara think they may be right about tech use increasing with higher degree groups. The mean portion of time using tech gets higher with each degree group. Leslie notices that the standard deviations are quite large and, for the less than high school group, the standard deviation is larger than the mean. She remembers back to Section \@ref(mean.spread) and thinks that a large spread might mean that the distribution is **platykurtic** with more observations in the tails than a normal distribution would have (giving it a flatter shape like the bill of a *platy*pus). 

Nancy thinks maybe a set of boxplots would provide a little bit of insight into how the data are distributed in each of the groups, and how the groups compare to each other.  
...
Leslie notices that a lot of people in the first two categorie selected zero percent of their time is on technology. She also notes that for all but the first category, there are a lot of people who selected 100% of their time is spent using technology. Leslie asks Kiara about these observations and how they might influence the results. Nancy says there are terms for measures where people are at the very top or very bottom of the range. When there are a lot of observations at the very bottom of a range, this is called a **floor effect**, while seeing a lot of observations at the top of a range is called a **ceiling effect**. 
...
Where $\alpha_{f}$ is the familywise Type I error rate, $\alpha_{i}$ is the individual $\alpha$ set as the statistical significance threshold, and $k$ is the number of comparisons. The formula for computing $k$ is $\frac{k\cdot(k-1)}{2}$, where $k$ is the total number of groups.

So, for the five group `DEGREE` variable where we set the $\alpha$ for each pairwise comparison at .05, the familywise $\alpha$ would be:
...
So the familywise error rate, or $\alpha_f$, indicates there is a 40% probability that a conclusion was wrong on at least one of the $t$-tests. To control this error rate, and for efficiency, use a single analysis of variance (ANOVA) test instead of 10 $t$-tests. A one-way ANOVA is useful for testing whether three or more means are equal. It can be used with two means, but the $t$-test is preferable because it is more straightforward.

### The $F$-test statistic for ANOVA

To compare mean age across the five degree categories, Nancy introduces Leslie to one of the ANOVA functions in R, `oneway.test()`. The `oneway.test()` function is in the *stats* library that loads with base R, so there are no new packages to install and load. The `oneway.test()` function has several arguments. The first argument is `formula =`, where the formula for testing would be entered. The formula for `oneway.test()` includes the continuous outcome variable first, then the tilde (` ~ `), then the categorical variable. So, the formula would be `continuous ~ categorical`. In this case, with the `USETECH` and `DEGREE` variables, it is `formula = USETECH ~ DEGREE`. 
...
```{r echo = FALSE, warning=FALSE,message=FALSE, fig.cap="Distribution of tech use by degree."}
...
gss.2018.cleaned %>%
  ...
  theme_minimal() + 
  labs(x = "Degree", y = "Percent Technology Used")
```

Leslie notices that the group mean for the junior college group (purple line) is higher than the overall mean (orange line). That is, the mean percent of time people use tech in this group is higher than the overall mean percent of time people use tech. Put differently, the null hypothesis of an ANOVA is that there is one mean to rule them all: the overall mean (orange line) does a better job explaining tech use for a particular group than the individual group means (purple lines). The alternative hypothesis is that there is *not* one mean to rule them all (sorry Lord of the Rings fans!): the individual group means (purple lines) do a better job explaining tech use *for that group* than the overall mean (orange line).

The difference between the group mean (purple lines) and the overall mean (orange lines) is *how much better* the group mean is at representing the group. This difference is used to compute the numerator of the $F$-statistic:
...
```{r echo = FALSE, warning = FALSE, echo = FALSE,fig.cap="F distribution (df = 4 and 1404) with shaded rejection region for .05."}
...
HO: The mean time spent on technology use is equal across degree groups. (one mean to rule them all)
...
The $p$-value in this case is `r format(techuse.by.deg$p.value, scientific = FALSE)`. The value of an $F$-statistic being this large or larger only happens a tiny percent of the time when the null hypothesis is true. 
...
Before turning to the results, Nancy clarifies the nesting portion. First, the `aov()` command was run. The output of `aov()` became the input of the `TukeyHSD()` function. Nested functions go from inner to outer. It's essentially a way of consolidating code (rather than two lines of code, we can write one line instead). Kiara warns that while nesting is often very useful, the code itself must still be readable in order to be reproducible.
...
```{r warning = FALSE, message = FALSE,fig.cap="Distribution of time using technology by degree for contrast."}
...
```{r warning = FALSE, message = FALSE,fig.cap="Distribution of tech use by degree for contrast comparing college to each of the other groups."}
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "Visualizing the distribution of use by degree for four planned comparisons."}
...
c1graph <- gss.2018.cleaned %>% 
  ...
  ggplot(aes(y = USETECH, x = DEGREE)) +
    ...
    scale_fill_manual(values = c("#78A678", "#7463AC"), guide = FALSE) +
    scale_color_manual(values = c("#78A678", "#7463AC"), guide = FALSE) +
    theme_minimal() + xlab("") + ylab("Percent of time\nusing tech")
...
c3graph <- gss.2018.cleaned %>% 
  ...
  ggplot(aes(y = USETECH, x = DEGREE)) +
    ...
    scale_fill_manual(values = c("#78A678", "#7463AC"), guide = FALSE) +
    scale_color_manual(values = c("#78A678", "#7463AC"), guide = FALSE) +
    theme_minimal() + xlab("Degree")+ ylab("Percent of time\nusing tech")
...
Where $n$ is the number of observations, $F$ is the $F$ statistic, and $k$ is the number of groups. 

Nancy jumps in with some R code to compute the $\omega^2$ for the technology use analyses. First she explains that the two functions they have used so far, `oneway.test()` and `aov()`, do not compute $\omega^2$ as part of the output. However, there are R packages that do compute it. Even so, Nancy recommends sticking with `aov()` and using the output from `aov()` to compute $\omega^2$ "by hand" rather than using a specific function. She shows the team how by examining the output from the earlier `aov()`:
...
The final thing Leslie wants to know is the cutoffs for small, medium, and large effects. Are they the same as for the other effect sizes? Kiara says unfortunately they are not, here are the cutoffs for $\omega^2$ effect size:
...
Describe how the interpretation of these findings might change if $\omega^2 = .04$. 
...
Leslie notes that none of the groups appear to be normally distributed based on their density plots and qq-plots. She also notices that the floor and ceiling values appear to be driving some of the non-normality. The Shapiro-Wilk test does not seem necessary in this case given the big deviations from normal in the density plots and qq-plots, however, Nancy suggests that they try it just to confirm. Leslie reminds herself that the Shapiro-Wilk test is testing the null hypothesis that the data are normally distributed. Nancy remembers that she had trouble before with using this test for multiple groups. She tries something new with the R code, which is to use the usual `summarize()` after `group_by()` but just print the $p$-value for the tests:
...
Nancy mentions that the _Levene's Test_ is widely used to test the assumption of equal variances. The null hypothesis is that _the variances are equal_ while the alternate is that at least two of the variances are different. The <span style="font-family:Lucida Console, monospace;font-weight:bold">car</span> package has a Levene's test command that can be used to test for equal variances:
...
```{r echo = FALSE, warning = FALSE, message = FALSE,fig.cap="Distribution of data violating an ANOVA assumption."}
...
$$
\begin{equation}
\overline{y}_{welch} = \frac{\sum\limits_{j=1}^k{w_k \overline{y}_{k}}}{\sum\limits_{j=1}^k{w_k}}
 (\#eq:weightwelch2)
\end{equation}
$$
...
$$
\begin{equation}
F_{welch}=\frac{\frac{\sum\limits_{j=1}^k{w_k(\overline{y}_k-\overline{y}_{welch})^2}}{r-1}}{1+\frac{2}{3}\Bigg(\frac{\frac{3\sum\limits_{j=1}^k\bigg(1-\frac{w_k}{\sum\limits_{j=1}^k{w_k}}\bigg)^2}{n_k-1}}{k^2-1}\Bigg)(k-2)}
 (\#eq:cov)
\end{equation}
$$
...
$$
\begin{equation}
df_{denom}=\frac{1}{\frac{3\sum\limits_{j=1}^k\bigg(1-\frac{w_k}{\sum\limits_{j=1}^k{w_k}}\bigg)^2}{n_k-1}}
 (\#eq:cov)
\end{equation}
$$

With fewer degrees of freedom, the $F$-statistic has to be a larger number to reach statistical significance. The R-Team examine a few $F$-distributions to see how this works:

```{r echo = FALSE, warning = FALSE, message = FALSE, fig.cap="F-distributions with 4 degrees of freedom in the numerator."}
...
dats$df <- factor(x = dats$df, levels = c(10, 25, 2000))
#levels(dats$df)
...
```{r warning = FALSE, message = FALSE,fig.cap="Ranks of technology use by degree groups."}
# graph the ranks
gss.2018.cleaned %>%
...
## Achievement 7: Understanding and conducting two-way ANOVA {#twowayanova}
...
```{r warning = FALSE, message=FALSE,fig.cap="Distribution of technology use by sex"}
...
```{r warning = FALSE, message = FALSE, fig.cap="Distribution of technology use by degree and sex"}
...
```{r warning = FALSE, message = FALSE, fig.cap = "Means plot of technology use by degree and sex."}
# means plots graph 
gss.2018.cleaned %>%
...
Nancy explains that the statistic column in the output is a column of $F$-statistics. Leslie notices that there are three $F$-statistics for this ANOVA. One for each of the two individual variables, also called **main effects** and one for the interaction term. Nancy explains that a main effect is the relationship between only one of the independent variables and the dependent variable; that is, they ignore the impact of any additional independent variables or interaction effects. In the results above, the `SEX` $F$-statistic is significant ($p$ < .05), indicating that there is a relationship between sex and technology use, even when ignoring the impact of `DEGREE` or the impact of the `DEGREE:SEX` interaction. Nancy says that this type of relationship is sometimes written as: "there is a main effect of sex on technology use". 
...
Leslie summarizes the findings:
...
Leslie looks at the `techuse.by.deg.sex` object in the environment pane and sees the residuals. She uses a Shapiro-Wilk test to check normality statistically and then plots the residuals for a visual assessment:
...
```{r warning = FALSE, message = FALSE, fig.cap = "Distribution of residuals from tech use by degree and sex ANOVA."}
# make a data frame
...
#### Achievement 2 recap: Understanding and conducting one-way analysis of variance (ANOVA)
...
#### Achievement 3 recap: Choosing and using post-hoc tests and contrasts
...
#### Achhievement 4 recap: Computing and interpreting effect sizes for ANOVA 

$\omega^2$ and $\eta^2$ are the two most common types of effect sizes for ANOVA. $\eta^2$ is the proportion of variance explained by the model (sum of squares - effect) relative to the total variance (sum of squares - total). However, $\omega^2$ is generally recommended over $\eta^2$ because it gets at the same idea but is less biased than $\eta^2$.

#### Achievement 5 recap: Testing ANOVA assumptions
...
#### Achievement 6 recap: Choosing and using alternative tests when ANOVA assumptions are not met
...
#### Achievement 7 recap: Understanding and conducting two-way ANOVA
...
A Type II error, also called $\beta$, is when there is a relationship but the study did not detect it (false negative). A Type I error, also called $\alpha$, is when there is *no relationship* but the study detects one (false positive). The $\alpha$ for Type I error is also the threshold set for statistical significance. The threshold for statistical significance is the amount of uncertainty tolerated in concluding whether or not a result is _statistically significant_. If $\alpha$ is .05, this is a willingness to risk a 5% chance of making a Type I error of rejecting the null hypothesis when it should be retained, or a 5% chance of gettign a false positive. Increasing sample size and decreasing the threshold for _statistical significance_ ($\alpha$) can aid in decreasing Type I and II errors. 
