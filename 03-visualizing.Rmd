...
```{r c3intro1, echo=FALSE, warning=FALSE, message=FALSE, fig.cap = "Reproduced figure showing Mortality Rate vs Funding from 2004 to 2015 for 30 Leading Causes of Death in the United States."}
...
```{r c3intro7, echo = FALSE, warning = FALSE, message = FALSE,fig.cap = "Firearm types manufactured in the US from 1990 to 2015."}
library(data.table)
...
gunsManuLong <- melt(gunsManu, id.vars = "Year",
                     measure.vars =
                       c("Pistols","Revolvers","Rifles","Shotguns",
                         "Total firearms"))
 #rename the vars
...
```{r c3main5,warning=FALSE,message=FALSE}
# open tidyverse
...
Kiara starts by showing Leslie the `ggplot()` command from the <span style="font-family:Lucida Console, monospace;font-weight:bold">ggplot2</span> package to create a bar plot of gun use. The <span style="font-family:Lucida Console, monospace;font-weight:bold">ggplot2</span> package uses the *Grammar of Graphics*, which is what the *gg* stands for. Kiara reminds Leslie that graphs build with `ggplot()` are built in layers. The first layer starts with `ggplot()` and `aes()` or **aesthetics**, which contains the basic information about how the graph should look such as: which variables are included in the graph and whether each variable should be represented on the x-axis, the y-axis, as a color, as a line type, or something else. The next layer typically gives the graph type, or graph **geometry**, in the grammar of graphics language, and starts with `geom_` followed by one of the available types (in geometry we study shapes -- what shape do you want to use on your graph?). In this case, Leslie is looking for a bar graph, so the `geom_bar()` is the geometry for this graph. Leslie remembers that `geom_bar()` is a layer of the plot and so is added with a `+` instead of a `%>%`. Leslie types: 
...
Before tackling the percent issue, Leslie works on the color. She remembers to add `fill = gun.use` to the aesthetics in `aes()`, and Kiara elaborates on the `aes()`. She says that if you want to change the way the graph looks *based on the data*, that change should be contained within the `aes()`. For example, Leslie wants the colors of the bars to be different *depending on category* which comes from the `gun.use` variable in the data set. Therefore, `fill = gun.use` should be within the `aes()` like this: `aes(x = gun.use, fill = gun.use)`. However, if Leslie just wanted to make the bars a color that she likes, like purple, Kiara explains that since this is *not* based on the data set, Leslie should put the `fill = gun.use` *outside* the `aes()` like this: `aes(x = gun.use), fill = "purple"`.

Leslie understands, but now can't remember how to manually change the colors of each bar. She goes back to her earlier code to find how to assign colors and delete the legend. She finds `scale_fill_manual(values = c("#78A678", '#7463AC'), guide = FALSE)` from the code used to make Figure \@ref(fig:c1main55) from the data preparation chapter and decides to use the same colors. Kiara reminds her that `theme_minimal()` will change the look of the graph so it is cleaner and uses less ink to print. Leslie adds in these options: 
...
Kiara suggests making a graph of the `AUQ310` variable in the `nhanes.2012` data set since it has more than two categories and so may be more interesting to view. `AUQ310` is the response to the question "How many total rounds have you ever fired?" for individuals who indicated they had used a gun. The audiology data codebook on the NHANES website [@NHANES20112012codebook] shows eight categories for `AUQ310`:
...
Kiara explains that all four of these plots can be made using different "shapes" or `geom`s in the `ggplot2` package. The data for this example is available at edge.sagepub.com/harris1e.
...
# make a histogram of funding and store it as the object "histo.funding"
...
The histogram shows frequency on the y-axis and mortality rate on the x-axis.
Leslie notices that the x-axis is shown using scientific notation. While she is familiar with scientific notation, which is useful for printing large numbers in small spaces, she knows it is not well-understood by most audiences (see Box \@ref(ch3leslie)). Kiara suggests changing the axis to show numbers that can be more easily interpreted; this can be done in several ways. One strategy is to convert the numbers from *dollars* to *billions of dollars* by dividing the Funding variable by 1,000,000,000. To plot a histogram in <span style="font-family:Lucida Console, monospace;font-weight:bold">ggplot2</span>, use the `geom_histogram()` layer.

```{r, message = FALSE, warning = FALSE, fig.cap="Research funding in billions for top 30 mortality causes in the US."}
...
She can now see the median funding level was about 2 billion based on where the think black line in the middle of the box is. Based on the boundaries of the box, she also determines that the middle half of the data appears to be between 1 and 5 billion dollars. 
...
Nancy suggests they plot the histogram, density plot, and boxplot together in order to see the similarities and differences between the three. Kiara has just the thing for that, the `grid.arrange()` command in the <span style="font-family:Lucida Console, monospace;font-weight:bold">gridExtra</span> package allows multiple graphs to be printed together. Leslie gives it a try, using the option `nrow = 3` to display one graph per row rather than side-by-side in columns.
...
One of the first questions Leslie has is whether men are more likely than women to have used a gun. Leslie notices that most of the mass shootings in the US have a male shooter and wonders if more men use guns overall? Nancy and Kiara think that examining whether men are more likely than women to have used a gun is a good question to answer using mosaic and bar plots.

Leslie already has the `gun.use` variable ready, but needs to know more about the sex variable. She looks in the codebook [@NHANES20112012codebook] to find how sex is coded. She finds that sex was likely measured by the `RIAGENDR` variable, which has the text, "Gender of the participant." Leslie assumes that this is the biological sex variable and looks at the way it was categorized:
...
Nancy thinks it is more clear with some color and titles and just recently found out the R-Ladies purple color hex code is `#88398a`. She and Leslie add a few options:
...
mosaic.gun.use.sex <- nhanes.2012.clean %>%
  ...
  scale_fill_manual(values=c('#88398a', "gray80"), 
                       guide = FALSE) +
  theme_minimal()
mosaic.gun.use.sex
```
...
group.gun.use.sex <- nhanes.2012.clean %>%
  ...
  geom_col(position = "dodge") +  # use `geom_col() to graph values themselves
  theme_minimal() +
  labs(x = "Participant sex", 
       y = "Percent in sex group") +
  scale_fill_manual(values = c("#88398a", 
                               "gray80"), 
                       name = "Firearm use")
group.gun.use.sex
...
Leslie is impressed! It looks like two density plots overlaid on top of each other. Nancy shows her the code and points out the `fill = gun.use` option in the `ggplot()` layer, which resulted in two plots with two colors. Nancy also points out the `alpha = .8` in the `geom_density()` layer. The alpha sets the level of transparency for color, where 1 is not transparent and 0 is completely transparent. The .8 level allows for some transparency while the colors still look about the same. She reminds Leslie that since the transparency level is not based on anything in the data set, the `alpha = .8` option should *not* be wrapped within the `aes()`. The rest of the graph is familiar to Leslie since it has the same options they have been using all day.
...
```{r echo = FALSE, warning=FALSE,message=FALSE,fig.cap = "Example of a uniform distribution."}
...
gun.use.age.md.err <- nhanes.2012.clean %>%
  ...
  ggplot(aes(x = gun.use, y = central,                           # use central tend for y-axis
             fill = gun.use)) +
  geom_col() +                                  # use the actual central value 
...
levels(fbi.deaths.cleaned$weapons)
```

Leslie notices that these categories are in alphabetical order, and points out the coincidence to Kiara and Nancy. Kiara remarks that this is no coincidence -- R will automatically put factor variables in alphabetical order! So reordering factors comes up quite frequently.
...
The team agrees that this graph does a good job of highlighting the handguns category and provides the additional information about the order of the weapons used in homicides. Hanguns is clearly the top most used weapon, with a mean of more than 6000 per year.

#### Point graph

Nancy shows Leslie that same data can be displayed with a single point rather than a bar. She copies the commands for the bar graph above, except now changing the `geom_bar()` layer to a `geom_point()` layer. 
...
As usual, Nancy is ready to move on. Before they do, Kiara looks over the code and asks about the `geom_jitter()` layer and why Nancy did not just use `geom_point()` like they have before when they want to see points on a graph? Nancy explains that `geom_jitter()` is a shortcut code for `geom_point(position = "jitter")`. The word "jitter" refers to being slightly irregularity, or unsteadiness -- just like a person who is nervous about something is sometimes described as being "jittery". Here, it means that the points are *slightly* shifted so that they're easier to see. That is, both `geom_jitter()` and `geom_point()` would do the same thing of placing the points on the graph, but by adding some "jitter", the are not all along a straight line. Having points all along a line makes it difficult to see the patterns, especially in large data sets where many of the data points may be overlapping. Kiara is satisfied with this and does not think they need to add more documentation.

#### Violin Plots

Earlier, violin plots were described as a happy medium between boxplots and density plots, but that they are typically used to look at the distribution of multiple categories (that is, the distribution of a continuous variable across multiple levels of a categorical variable). Leslie copies the from above, removing the `geom_jitter`, and changing `geom_boxplot` to `geom_violin`.
...
Leslie, Nancy, and Kiara agree that this was not useful at all because, as they learned from the other plots above, there are so few cases per group. Nancy still wants to illustrate to Leslie the utility of violin plots because in many scenarios, the are indeed helpful. She writes some quick code using the `nhanes.2012.clean` data from above to look at whether the distributions of age are the same for males and females.

```{r}
nhanes.2012.clean %>%
  ggplot(aes(x = sex, y = RIDAGEYR)) +
  geom_violin(aes(color = sex), fill = "white") +
  theme_minimal()
```

Although there doesn't seem to be much of a difference in the distribution of ages between males and females, Leslie sees how this type of plot might be useful for future projects.

Before they move to the last section, Leslie wants to look at the the different graph types of the weapons used in homocide one last time. She copies her code from the previous section and adds the new graph names:
...
```{r warning = FALSE,message = FALSE,fig.cap="Research funding for top 30 causes of mortality in the US."}
#scatterplot of gun research by funding
#with axes showing a natural log scale
scatter.gun.funding <- gun.research %>%
...
To download the NHANES data for the examples in this chapter, install the <span style="font-family:Lucida Console, monospace;font-weight:bold">RNHANES</span> package and run the code beloW: 
