...
Leslie first wants to clarify the definition of transgender to make sure they are all on the same page. First, there is a difference between sex and gender that is important for understanding the term transgender and other terms related to gender identity and sexual orientation. The term _sex_ refers to biology and the term _gender_ refers to identity. Nancy agrees with the definition of sex, but had heard broader definitions of _gender_ that included biological, psychosocial, and cultural factors along with identity [@mayer2008sexual]. She looks up the World Health Organization (WHO) definition for gender:
...
Leslie is glad they looked this up. She goes on to explain that *transgender* people are people whose *biological sex* is not consistent with their *gender* [@mayer2008sexual]. She quotes an older article from 2008 that she thinks has a clearer definition:
...
The 2008 paper gives a few examples of transgender including a person who is biologically female but identifies as a man, a person who is biologically male but identifies as a woman, or someone who feels like both or neither gender [@mayer2008sexual]. Nancy looks at the Centers for Disease Control and Prevention (CDC) website and explains that gender identity is the gender a person identifies as and gender expression is the gender a person outwardly shows [@TransgenderPersons]. When describing individuals, typically the word of the person's chosen identity is included. That is, a person that is biologically female but identifies as a man would be described as a "transgender man". Now that they have a clearer idea of who is considered transgender, Leslie gives some background on health and healthcare for transgender people to Nancy and Kiara.

Transgender people face serious physical and mental health disparities [@safer2016barriers], often related to discrimination [@bradford2013experiences]. Among the most prevalent and concerning disparities are extremely high rates of HIV and other sexually transmitted infections, poor mental health, and high rates of substance use and abuse. For example, transgender women have a 19.1% prevalence of HIV worldwide [@baral2013worldwide] while reproductive aged adults have an HIV prevalence of .4%. A survey of transgender adults in the United States found 44.1% have clinical depression, 33.2% have anxiety, and 27.5% have somatization symptoms (e.g., physical symptoms stemming from depression, anxiety, or another psychiatric condition) [@lombardi2010transgender]. Attempted suicide rates range from 32% to 50% of transgender people worldwide, with higher rates among younger age groups [@virupaksha2016suicide]. 

In 2011, an Institute of Medicine (IOM) report found 35.5% of transgender adults are current smokers; this is more than twice the percentage of cisgender (e.g., people whose gender identity matches their biological sex) adults (14.9% smoking rate) [@stratton2001institute]. Transgender adults also have higher rates of alcohol and substance abuse [@benotsch2013non; @santos2014alcohol] and of overweight and obesity [@boehmer2007overweight]. Exacerbating these health disparities are limitations on access to healthcare [@RefWorks:2299] and limitations of the healthcare system to effectively address the unique health needs of transgender people [@reisner2016global; @daniel2015lesbian]. 

Given the high smoking rate, substance and alcohol use, and the prevalence of overweight and obesity, it is important that transgender people are adequately screened for cancer and other health conditions. However, Leslie found some conflicting results on this topic. One study from **INSERT CORRECT YEAR HERE** found that transgender adults had a 47% lower odds of breast cancer screening than cisgender adults attending an urban clinic [@bazzi2015adherence] **CHECK THIS CITATION!**. However, a different study from **INSERT CORRECT YEAR HERE** reported that transgender and cisgender adults have comparable rates of mammography screening [@RefWorks:2300] **CHECK THIS CITATION!!**. **Nancy thinks maybe the rate is increasing over time since the more recent study uses slightly newer data, but Kiara suggests that the Behavioral Risk Factor Surveillance Survey (BRFSS) data used in the second study likely measures people with different characteristics than the 2015 study which relied on people seeking care at an urban clinic.** Both of these factors, may contribute to the higher rate of screening in the more recent study. 

Leslie looks up the BRFSS and finds that it is a large national survey conducted by the CDC in the US by phone every year [@CDCBRFSS-2019-04-22]. The 2017 mammogram paper analyzed data from participants in the 2014 BRFSS. The 2014 BRFSS included 464,664 participants and the sampling process was designed so that the sample would represent the US population as much as possible. The survey included questions about characteristics like age, sex, and income, along with questions about health behaviors and outcomes. 
...
```{r c2intro1, echo=FALSE, warning=FALSE,message=FALSE}
library("knitr")
library("kableExtra")
library("tidyverse")
...
Before getting started, the team looks through the 2014 BRFSS codebook [@brfss-2014-codebook] together (which can be found on the CDC BRFSS website [@CDCBRFSS-2019-04-22]), and finds the transgender transition status information on page 83. The frequencies in this frequency distribution are 363 MtF transgender, 212 FtM transgender, 116 gender nonconforming, 150,765 not transgender, 1,138 don't know/not sure, 1,468 refused, and 310,602 not asked or missing.
...
While Leslie is distracted, Nancy sees her chance to do some of the coding. She slides the laptop over and changes the `mutate()` code to use `recode_factor()` with the category labels. When she is done, she shares her work with Leslie and shows her that `recode_factor()` requires the original value of the variable on the left side of the `=` and that the original values are enclosed in backticks (  \``value`\`  ) because the values are considered **names** rather than quantities. In R, **names** are labels given to a category or a variable or another object. Names that begin with a number are enclosed in backticks (or quote marks) in order to be recognized by R as a name and not a quantity or value. 
...
Leslie wonders aloud how she would ever be able to remember the rule about putting numbers in backticks for `recode_factor()`. Nancy says that some code is easy to remember, especially if she uses it a lot, but otherwise she uses the help documentation almost every day she is coding. She reminds Leslie that the help documentation is easy to use directly from R Studio and can be found as one of the tabs in the bottom right pane. She shows Leslie how to type "recode_factor" in the search box at the top of the help pane and the documentation for `recode_factor()` appears in the pane. She also tells Leslie that you can type a single "?" and then the name of the function into the Console, and the help page will appear (`?recode_factor`).
...
It looks better, but it still needs some work on the formatting before it is ready to share with others. Leslie starts thinking about the `summary()` command from their last session, and wonders if that might also be a good way to get frequencies. Nancy thinks this is an interesting idea and they decide to try using `summary()` and saving the results as a data frame:
...
After looking over the results, the team decides that the `desc.from.summary` table and the `desc.from.summary.df` data frame do not look very easy to use. Leslie wonders if there is any other code like `summary()` that summarizes all the variables at once. Nancy says she has used `describe()` from the <span style="font-family:Lucida Console, monospace;font-weight:bold">Hmisc</span> package. Leslie recalls that a package must be installed once (you need an internet connection to do so), but after it's installed there are two ways to access the functions that come from the package. The first way is to load the entire package (kind of like opening Microsoft PowerPoint for slides) using `library(Hmisc)`. Yet since they only want to use the one `describe()` command, Nancy suggests using the `::` structure instead. Leslie gives it a shot:
...
They look through the information and see that percents are included, which is something that is not in the `summary()` output, however, there is a lot of output to read through in order to find specific information. Kiara says she prefers to use more targeted statistics and just uses `summary()` and `describe()` to get a sense of the data or to confirm her data management tasks have worked. Leslie thinks this is a good strategy, although she is curious about alternatives from other packages.  

Nancy says that the <span style="font-family:Lucida Console, monospace;font-weight:bold">descr</span> (short for descriptives) package has a good option for a basic frequencies table with the `freq()` command. Nancy also knows that a graph is automatically printed with the `freq()` output and it is not useful at this point, so she shows Leslie how she uses the `plot = FALSE` option to stop the graph from printing with the output.
...
Leslie notices that there are two columns in the output that show percents. She reviews the columns and finds that the *Percent* column includes the missing data (`NA`s) in the calculation of the percent of observations in each category. The *Valid Percent* column removes the `NA` values and calculates the percent of observations that falls into each category *excluding the observations missing values on this variable*. 

Leslie thinks this is ok, but since they are already using the <span style="font-family:Lucida Console, monospace;font-weight:bold">tidyverse</span> package, they might try using it for this instead of opening a new package. She writes some alternate code to show Leslie how she might create this table:

```{r c2main8, warning=FALSE}
...
Leslie is not impressed. This looks like a lot more work to get the same information. Nancy agrees that, in this situation, the code seems more complicated to get the same table. She explains that sometimes it might be worth the time to use the more complicated code in order to keep a consistent style or in order to be able to use the code or its output for other things later. Leslie nods but remains skeptical. Kiara points out that this is an example of how you can do the same thing in a number or different ways in R. There is usually no one "correct" way. 

Nancy notes that while both the `freq()` and <span style="font-family:Lucida Console, monospace;font-weight:bold">tidyverse</span> tables include the information you might need for reporting, they are still not formatted in a way that can be directly incorporated into a report or other document. Leslie suggests that they copy these results into a word processing program and format the table there. 

Kiara cringes at this suggestion. Copying from statistical output into another document is where many errors occur in scientific reporting [@RefWorks:2301] and is a major reason why research results are not reproducible. It is more efficient and accurate to create a formatted table in R and use it without transcribing or changing the contents. Nancy suggests there are two great options for creating reproducible tables directly in R using the <span style="font-family:Lucida Console, monospace;font-weight:bold">tableone</span> and <span style="font-family:Lucida Console, monospace;font-weight:bold">kable</span> packages (see Box \@ref(ch2nancy2). Kiara promises to go through making pretty tables that can be viewed outside of R in a bit, but for now the R-Team is happy with `desc::freq(brfss.2014.cleaned$TRNSGNDR, plot = FALSE)` code. 
...
Now that they have a good table of descriptive statistics for a factor variable, Nancy and Leslie move on to numeric variables. Numeric variables are measured on a continuum, and can be truly continuous or just close to continuous. Leslie knows these types of variables are not well described using frequency distributions. For example, a frequency table of the age variable (column `X_AGE80`) looks like this:
...
$$
\begin{equation}
m_x=\frac{\sum\limits_{i=1}^n{x_{i}}}{n}
 (\#eq:mean)
\end{equation}
$$
...
However, Nancy explains, if Bill Gates just happened to be a friend of yours, you would include his $11.5 billion dollar salary by adding it to the existing `salaries` vector. Adding a value to a vector can be done by concatenating the existing values with the new values in a new vector using `c()`, like this:
...
In this case, the median is not too far from the mean. Leslie is curious about how different from normal a distribution has to be before people use the median instead of the mean. From the examples so far, she thinks that median salary is definitely a more accurate representation of the middle of the `salaries` vector than the mean of `salaries`. However, for the two skewed distribution histogram examples in the figure, the median and mean are very close to each other and, since the mean seems like a more widely used and understood measure, might it be a better choice?
...
$$
\begin{equation}
skewness_x=\frac{1}{n}\sum\limits_{i=1}^n\Bigg(\frac{{({x_i-m_{x}}})}{s}\Bigg)^3
 (\#eq:skew)
\end{equation}
$$
In this equation, $n$ is the sample size, $x_{i}$ is each observation of $x$, $m_{x}$ is the mean of $x$, and $s$ is the sample standard deviation. Let's break this down. The first thing to note is that the mean of $x$ ($m_{x}$) is subtracted from each observation of $x$ ($x_{i}$). These are called **deviation scores**. Next, each deviation score is divided by the standard deviation, $s$, and the result is cubed. Finally, we take the average of these cubed values to get the skewness of $x$. The resulting value is usually *negative* when the skew is to the left and usually *positive* when the skew is toward the right. Because skewness is strongly impacted by the sample size, the value of skew considered to indicate a serious departure from normality differs as sample size differs. 
...
There is one salary of \$25,000, 2 salaries of \$41,000, and so on. With data this simple, we could stop here. However, most data sets will be much larger and it will be more efficient to have a set of R commands to use. Nancy therefore puts the table in order from largest frequency to smallest frequency using `sort()` with `decreasing = TRUE`, which puts the salaries in order from lowest to highest:
...
Finally, to just print the first category name, use the square brackets, **`[ ]`**. In R, the squared brackets are used to grab a particular location (sometimes called an "index"). For example, the `salaries` vector has 5 observations: `r salaries`. To get the 4th observation of `salaries`, you can type `salaries[4]`, which is `r salaries[4]`. Here, to get the first category name, we can use the same bit of code, and then add a `[1]` to the end: 
...
Everything looks good, so she shows Leslie how to make a histogram from the cleaned data. The code is similar to what they used at the end of the data preparation day, but the `geom_` layer this time is `geom_histogram()` to make a histogram (you can remember that the `geom_` is like the plot's "geometry", or what shape you want to use in your plot):
...
Well that didn't work very well! The mode looks like 0 unhealthy days. The mean and the median show `NA`, though. Nancy explains that the `mean()` and `median()` commands require removal of `NA` values. To fix this, she needs to include a second argument that tells the `mean()` command how to treat missing data. She adds the option `na.rm = TRUE`, where "na" stands for `NA` and "rm" stands for "remove":
...
Leslie admits that she is concerned that she'll never be able to remember all the arguments for the different commands. Kiara reassures her that she does *not* need to memorize them! First, Kiara explains, Leslie can always check the Help tab in the lower right corner to look up any function. The help documentation will always list what the function does, the arguments it can take, what the output of the function means, and there are even examples at the very bottom for how to use the particular function!

Additionally, Kiara shows Leslie how to use tab completion to make her life easier. Kiara tells Leslie to start type the letters "me" in the Console and then hit her laptop's *tab* key. Leslie notices that a lot of different commands starting with "me" appear, and that `mean()` is first! Since `mean()` is first and highlighted, Kiara tells Leslie to now hit the *enter* or *return* key. Leslie is amazed that she didn't need to type out the rest of the word "mean" *and* that the parentheses autmoatically appeared! Neat! Now, Kiara tells Leslie to hit *tab* once again. The first argument `x` appears, and a brief description of that argument. Here, `x` is the variable that you want to get the mean of. Leslie hits *enter* and now the `x = ` argument appears in her code and is ready to be filled in with `brfss.2014.cleaned$PHYSHLTH`. Leslie gets the inkling to start typing the letters "brf" and hits *tab*. Sure enough, she sees the correct data frame, highlights it via up/down arrow keys, hits *enter*, then adds the `$` and again tab-completes until the full variable name is printed. Kiara explains that arguments within a function are separated by commas. She asks Leslie to put a comma after the variable name, and press *tab*. Leslie now sees a `na.rm` argument appear! Leslie finishes the line of code via tab-complete, and is thrilled that she can reduce her number of keystrokes!

Nancy gets them back on track with descriptives. Kiara suggests that <span style="font-family:Lucida Console, monospace;font-weight:bold">tidyverse</span> code might be clearer here since the name of the data frame would not have to be included in all three commands. To pipe the data frame into the central tendency commands, use `summarise()`. *(Fun Fact: You will see 'tidyverse` code use British spellings, however the American spellings work, too! Ex: `summarize()` is the same as `summarise()`.)*
...
$$
\begin{equation}
s^2=\frac{{\sum\limits_{i=1}^n}({x_i-m_{x}})^2}{n-1}
 (\#eq:variance)
\end{equation}
$$

Leslie sees that the $s^2$ is the variance, the $\sum$ symbol is sum, the $x_i$ is each individual value of $x$, the $m_{x}$ is the mean of $x$, and $n$ is the sample size. So, the variance is the sum of the squared differences between each value of $x$ and the mean of $x$ (or the sum of squared deviation scores) divided by the sample size minus one.
...
$$
\begin{equation}
s=\sqrt{\frac{{\sum\limits_{i=1}^n}({x_i-m_{x}})^2}{n-1}}
 (\#eq:stddev)
\end{equation}
$$
...
Leslie has this feeling that there is something else they need to look at before they are done discussing normal distributions. Kiara suggest that maybe she is remembering **kurtosis**. Kiara explains that kurtosis is about how many observations are in the tails of a distribution. She says that some distributions look bell shaped, but have a lot of observations in the tails (platykurtic) or very few observations in the tails (leptokurtic) [@westfall2014]. Leslie asks Kiara whether she is remembering correctly that **platykurtic** distributions are more *flat* (as in a *platy*pus has a flat beak) and **leptokurtic** distributions are more *pointy*. Kiara says this is a common way of describing the shapes of these distributions, but that, technically, kurtosis measures whether there are many or few observations in the tails of the distribution [@westfall2014].
...
$$
\begin{equation}
kurtosis=\frac{1}{n}\sum_{i=1}^{n}{\bigg(\frac{x_i-m_x}{s}\bigg)^4}
 (\#eq:kurtosis)
\end{equation}
$$

Leslie reads that, in this equation [@westfall2014], $n$ is the sample size, $s$ is the standard deviation of $x$, $x_i$ is each value of $x$, and $m_x$ is the mean of $x$. The $\sum\limits_{i=1}^{n}$ symbol indicates the values from the first value of $x$ ($i = 1$) to the last value of $x$ $i = n$ should be summed. A normal distribution will have an $kurtosis = 3$; distributions with $kurtosis = 3$ are described as **mesokurtic**. If $kurtosis$ is above or below $3$, there is excess kurtosis. Values of kurtosis above 3 indicate the distribution is *leptokurtic*, with fewer observations in the tails than a normal distribution would have given the mean, standard deviation, and sample size (the fewer observations in the tails gives it the pointy "look"). Values of kurtosis below 3 indicate the distribution is *platykurtic*, with more observations in the tails than a normal distribution would have given the mean, standard deviation, and sample size. 

Leslie looks at the <span style="font-family:Lucida Console, monospace;font-weight:bold">semTools</span> package documentation and finds `kurtosis()` to compute the kurtosis and a few related values. The `kurtosis()` command subtracts 3 from the kurtosis, so positive values will indicate a leptokurtic distribution and negative will indicate a platykurtic distribution. The same cutoff values from skew also apply for the $z$ for small, medium, and large sample sizes in kurtosis. Leslie loads package and tries it for the two variables used in the leptokurtic and platykurtic graph above: 
...
The range does not seem too informative on these graphs, Leslie notes, because it marks the very highest and very lowest values for a variable but no indication of how the observations are spread out between these values. In order to get the exact values, Nancy jumps in and writes the code to compute the range for the `PHYSHLTH` variable:
...
Leslie notices that they have typed `na.rm = TRUE` for all but one of the statistics in the `summarise()` code. She asks Nancy if there is some way to remove `NA`s, and make the code less repetitive. Nancy is delighted that Leslie is starting to think about the format of her code. She says that there is a way to omit `NA` values using `na.omit()` with the name of the variable of interest, like this:
...
Leslie examines the statistics so far. She notes that the IQR value makes sense; there is a 3 day difference between the first and third quartiles for unhealthy days. She remembers she has usually seen the IQR reported without much interpretation, like this: IQR = 3. 
...
brfss.2014.cleaned %>%
  na.omit(PHYSHLTH) %>%
  ggplot(aes(x = PHYSHLTH)) + 
  geom_histogram(color="gray60", fill="white", binwidth = 1) + 
...
They share their finding with Nancy, who is playing on her phone because she has run out of things to code for the moment. Nancy has not used the **B index**, but finds `B()` in the <span style="font-family:Lucida Console, monospace;font-weight:bold">qualvar<span> package. She first looks up the help documentation for the `B()` command, and sees that the first argument is `x`, which is described as "a vector of frequencies". Frequencies are numbers, so the `x` argument of the `B()` command needs to be a numeric variable. Nancy checks the `TRNSGNDR` variable type using the `class()` command and finds that it is a factor. Fortunately, she can use the `as.numeric()` command to change the factor to numeric for the `B()` command.
...
Kiara shares one of her favorite data management tips with Leslie: create a smaller data set to work with by limiting the data to the observations that will be analyzed. There are several ways to subset a data set to only include particular observations. One method is to simply grab the observations wanted using the square brackets `[ ]`. Kiara reminds Leslie that she already used `[ ]` to get the 4th element of the `salaries` vector using `salaries[4]`. Vectors are one-dimensional, and therefore you only need one number inside the brackets. Data frames are two-dimensional, Kiara explains, since there are both rows and columns. So to index a data frame, the square brackets requires two numbers, separated by a comma, and the order will always be `[rows,columns]`. Some quick examples:

- Get the value of the 3rd row and 2nd column: `data.frame[3,2]`
- Get all of the values in the 5th column: `data.fame[,5]` *nothing in the rows part means "all", but it still needs to be separated by a comma*
- Get the values for rows 2 though 6 and columns 4 through 10: `data.frame[2:6,4:10]` *the colon `:` can be read as "through"*
- Get the values for rows 1, 7, 18 and columns 4 through 10: `data.frame[c(1,7,18), 4:10]`

Another method of subsetting is the `subset()` function. However, since the R-Team has been using <span style="font-family:Lucida Console, monospace;font-weight:bold">tidyverse</span>, they decide to subset via the `filter()` command, and Nancy suggests that Leslie look up the help documentation for the `subset()` function on her own. According to the "Description" in the `filter()` help page, this function "choose[s] rows/cases where conditions are true". Put differently, the coder writes a statement that must evaluate to either `TRUE` or `FALSE` -- the result of the statment must be logical. Nancy knows the importance of understanding logical statements, so she gives Leslie several examples (you can go ahead and try these out in the Console!):

- Is the object on the left the equal to the object on the right? Use `==`
    - `3 == 9` evaluates to `FALSE` because 3 does not equal 9
    - if `a = 10` and `b = 10`, then `a == b` evaluates to `TRUE`
    
- Is the object on the left *not* equal to the object on the right? Use `!=`
    - `3 != 9` evaluates to `TRUE` because 3 does not equal 9
    - if `a = 10` and `b = 10`, then `a != b` evaluates to `FALSE`
...
Nancy then tells Leslie that she can combine these logical statements! Take the same `3 < 9` and if `a = 10` and `b = 10`, then `a == b` as above. To find out if *both* conditions are true, use the `&` for "and". `3 < 9 & a == b` evaluates to `TRUE`. This is because *both* statements are true. It reads "3 is less than 9 *and* a is equal to b". However, `3 > 9 & a == b` evaluates to `FALSE`. This is because the first statement `3 > 9` is `FALSE`, therefore it doesn't matter that the second part of the statement (`a == b`) is still OK -- the whole combined statement evaluates as `FALSE`. 

Nancy wraps up by saying that if you want to know if one *or* the other is true, Leslie can use the `|` symbol. `3 > 9 & a == b` now evaluates to `TRUE` because one of these is `TRUE` (`a == b`). 

Now that Leslie has a better grasp of logical conditions, the R-Team gets back to the task at hand using `filter()`. Leslie also notices that the `_AGEG5YR` variable was imported with an `X` at the beginning of the variable name, so she incorporates this into the code:

```{r c2main46}
# BONUS: check the column names of a data frame
# colnames(brfss.2014.cleaned)
...
The resulting smaller data frame now includes 222 individuals. Leslie notices that the breast cancer screening article shows 220 people who fit the criteria and wonders why she and Nancy have 2 additional people in their data frame. She decides she will review the percentages closely in creating the table to see where these 2 people fit in, and determine if they should be excluded. 
...
**SHELLY GOT RID OF LAPPLY IN THE CODE BELOW, SO NOT SURE WHAT TO DO WITH THIS SENTENCE (AND BOX REFERENCE**Luckily, Nancy knows a command in the `apply()` family of commands (see Box \@ref(ch2nancy1)) for more on `apply()` commands) that can be used to change all the variables in this small data set to factor types:
...
**EXPLAIN MUTATE_ALL HERE INSTEAD OF LAPPLY**She explains to Leslie that the command `lapply()` is used to apply some function to every item in a list (list + apply = lapply). Since a data frame is a type of list made up of variables, `lapply()` can be used to change each item (each variable in this case) to a factor. When using `lapply()` with the `%>%`, the data frame name is already passed through the `%>%`, but `lapply()` still needs a placeholder, so `.` is used as a dummy name.  
...
This is a really tricky data management problem! Leslie asks if they can change `TRNSGNDR` to `NA` for when the `HADMAM` variable is category 9, which is the code for "Refused." This seems to Kiara like it might work, but it will require a new command. The first one that comes to Kiara from her data management work is `if_else()`**FOOTNOTE?-SEE SHELLY'S COMMENTS**. The `if_else()` code takes three arguments. The first argument is a logical statement (or condition) that must evalue to either `TRUE` or `FALSE` -- just like the `filter()` function from above! The second argument of `if_else()`, is `true = `. This is where you tell R what to do if the statement from the first agrument evaluates as `TRUE`. The third argument, `false = ` is what you want to happen if the statement evaluates as `FALSE`. The second and third arguments have to be the same data type. Before coding, Kiara first has Leslie say outloud what she wants the `if_else()` command to do, so that it's easier for Leslie to code. She responds with:

>"For each person in the data set, if that person's value in `HADMAM` is *not* equal to category 9, then leave their `TRNSGNDR` value as is (do nothing). For everyone else that *does* have a value of 9 in `HADMAM`, change their corresonding `TRNSGNDR` value to be `NA`".

Kiara and Nancy look at each other and are quite impressed with Leslie! So they add a line to the data management before `select()` and test it: 
...
Great! The mean and standard deviation are added to the table! Kiara reminds Leslie that the mean and standard deviation are only good measures for the continuous variables when the variables are **normally distributed.** Leslie remembers that they looked at the distribution of `PHYSHLTH` with a histogram. She copies the code to take another look:

```{r c2main63, message = FALSE, warning = FALSE, fig.cap="Distribution of PHYSHLTH variable."}
# make a histogram 
brfss.2014.small %>% 
  ggplot(aes(x = PHYSHLTH)) + 
  geom_histogram() 
...
The R-Team is happy with the final product. Nancy explains that there is an R package called <span style="font-family:Lucida Console, monospace;font-weight:bold">kable</span> that is more difficult to use but also more flexible to format. Nancy wants to show Leslie the alternate way using <span style="font-family:Lucida Console, monospace;font-weight:bold">kable</span>, but it seems like it might be overwhelming. Leslie says she is up for it after a quick coffee break `r emo::ji("coffee")`. Kiara says that sounds perfect and they caffeinate and regroup to figure out a <span style="font-family:Lucida Console, monospace;font-weight:bold">kable</span> table.
...
Leslie notes that `rbind()` works great to create a data frame with each category and the percent in that category. However, with seven variables to work with, Nancy suggests that they be more efficient by using a **for loop**. Nancy explains that for loops are a fundamental aspect of computer programming, and you can find them in all programming languages -- not just R! They are a general purpose way of doing the same thing multiple times, or doing something **iteratively**. A for loop can be read as "for each item in (things you want to iterate over), do something". This seems a little abstract to Leslie, so nancy suggests showing the basic format and a very basic example. 

```{r c2main68a, eval=FALSE}
...
Nancy breaks this down for Leslie:

* The `1:some number` bit contains the elements you want to iterate over. Here, we want to perform the same function for the 7 variables from our `brfss.2014.small` data.frame. So we could write `1:7`. However, sometimes you don't know how many iterations to do. For example, if you knew you wanted to do the same thing for each row, but you don't know off the top of your head how many rows there are, you could do `1:nrow(data.frame)`. 
...
* The curly brackets **`{ }`** indicate that you are now in the body of the for loop. Whatever it is you want done a lot of times, you put that in here. 

* Although not shown here, most of the time we want to store the output. For this, you simply need to make an *empty* object that will ultimately store the output. This is called *initializing*. 

Nancy thinks it's time to go through a basic example:

```{r}
# basic example of a for loop
squaredNumbers <- NULL # initialize an empty vector that will contain the output

for (i in 1:10) { 
  squaredNumbers[i] <- i^2 # body of the for loop
}
...
Leslie tries to verbalizes what just happened: 

>First, make an empty vectory called `squaredNumbers`. For each item in numbers 1 though 10, take that number or `i` and square it (`i^2`). Whatever that is, store it in the `squaredNumbers` vector.

Nancy and Kiara are again impressed, but Leslie has one question. Why is there a `[i]` next to `squaredNumbers`? Great question! Nancy reminds Leslie that `i` is the element. So on the first iteration, `i=1`. The `squaredNumbers[i]` bit is saying to store the result of the 1st iteration in the in the 1st location of `squaredNumbers`. On the 7th iteration, `i=7`, so we want to make sure we're storing that result in the 7th location of the output `squaredNumbers`. Leslie thinks she understands, but wants to practice on the actual data set at hand. She is reminded that the goal is to get a data frame that contains percent per category of all 7 factors in the `brfss.2014.small` data set. Put differently "for each variable (or column) in `brfss.2014.small`, get the percents via the `prop.table` function, and then combine the results into a single data.frame." Nancy begins to write the for loop. 
...
This is really close to ready! But there are still a couple details to work out. First, it's hard to tell which variable (here, which iteration), each row belongs to. Nancy suggests that they redo the for loop, this time including the iteration (here, the variable/column name) as it's own column in the output `table.data`. Also, Leslie remembers that the `brfss.2014.small` data frame includes the `PHYSHLTH` variable, which is not in the table they are trying to recreate, but it is currently in this table. Leslie notes that `PHYSHLTH` is the last column. Nancy gets to work!
...
for (i in 1:(ncol(brfss.2014.small)-1)) {
  ...
  # combine the label.names data frame and table.each data.fram via columns
...
Leslie thinks this is perfect, although Kiara asks her to take a closer look at the newly created `table.data` and the original published table because there is one slight difference. Leslie finds what Kiara was hinting at -- the original table does not include the *No* category from the healthcare variable. Leslie thinks this is no problem and goes about subsetting the `table.data` to grab the first 26 rows (or not the last row) and all of the columns. Leslie also labels the columns of the data frame to be consistent with the table columns.

```{r}
table.data <- table.data[c(1:26),]

colnames(table.data) <- c("Variable",
...
Perfect! Leslie recognizes the importance of for loops -- it's much easier than copying/pasting the same code over and over again! Nancy tells Leslie that while for loops can be done in R and in most other programming languages, there are also some R-specific functions that act as for loops. She mentions the `apply` family of functions, and tells Leslie she can read more about it at Nancy's Fancy Code (**insert link to Nancy's fancy code**). 

Before moving on to formatting tables, Nancy wants to mention one other thing. She explains that if Leslie thinks that the same bit of code would be useful across many different analyses, she might consider making a custom **function**. The R-Team has been using functions, or commands, all along! They are the "verbs" of the programming language and perform actions. For example, `mutate()` is a function that comes from the `dplyr` package. There are three parts of a function: a unique name (well, mostly unique -- see the "namespace" issue from Chapter 1), input arguments, and an output. So the function name is `mutate()`, the input arguments are the data to be transformed and how they should be transformed, and the output is the newly transformed data set that can be stored as it's own object. One of the reasons R is so powerful, Nancy explains, is that lots of really talented people have already made lots of functions, and bundled them into packages. However, if you write a piece of code that you think you will use often, you can make your own custom function. It works the same way as it does for functions that come from a package. So, if you think you there might be cases in the future where you'll want to make this kind of table, even if it's on a totally different data set and for a totally different project, you can write a custom function so you don't need to re-type the same bit of code over and over again. 

Leslie is really excited but also overwhelmed by this idea, and the caffeine is starting to fade. Rather than trying to power through it, Nancy suggests Leslie check out **Nancy's Fancy Code**, along with searching various internet forums for more on making custom functions. 

Leslie still seems a bit uneasy on how she's going to remember everything she's learned. Nancy reassures her that all of the commands used here will be used and explained multiple times throughout the text, so not to worry if everything is not yet 100% clear as there will be additional opportunities to use these and all the other commands. Nancy suggests copying and pasting any new commands Leslie is trying to use and changing little things to see how the results change. In this way the purpose of each little piece of the code might become more familiar. Nancy also suggests that the help documentation and Google can be useful for finding examples of code in the early stages of learning R. 
...
Well, it's a start, says Leslie. She then tries removing the column `Variable` using subsetting:

```{r c2main74}
# make a table with the table.data, but use only columns 2 & 3
table.data[,2:3] %>% 
  kable(format = "html", caption = "Transgender Survey Participant Demographics (n=222)") 
```
...
# make the table with table.data (after subsetting) and make better
# row names. Note that this is a slightly differentw way of subsetting
table.data[,-1] %>%
  kable(format = "html", caption = "Transgender Survey Participant Demographics (n=222)",
        row.names = FALSE) %>%
...
# make the table with table.data (after subsetting) and make better
# row names.
# Note this slightly different way of subsetting - lots of ways to do the same
# thing in R!
table.data[,-1] %>%
  kable(format = "html", caption = "Transgender Survey Participant Demographics (n=222)",
      row.names = FALSE) %>%
...
Now that she has the manuscript table reproduced, Leslie is interested in making the bigger table with frequencies. The first step, says Nancy, is to modify the for loop to compute both frequencies and percentages. The `table()` command computes frequencies. If stored as a data frame object, then you can combine the frequencies to the percentages table in a column-wise fashion using `cbind()`.

```{r c2main77}
# revise the for loop to compute both
...
for (i in 1:(ncol(brfss.2014.small)-1)) {
  # first, get the table with precents
  table.each <-
    data.frame(round(x = prop.table(x = table(brfss.2014.small[, i])) * 100, 1))
  
  # Now get the frequencies using table
  frequencies <- as.data.frame(table(brfss.2014.small[,i]))
  
  # Add this column of frequencies to the table.each data frame
  table.each <- cbind(table.each, frequencies$Freq)
...
#rename the columns
names(x = table.data) <- c("Variable", "Survey participant demographics", "Percent", "Frequency")

# while usually not necessary, columns can be reordered
# it works almost exactly the same as subsetting, except now you
# choose the order instead of removing a position. 
# to swap Percent and Frequency:
table.data <- table.data[,c(1,2,4,3)]

# the above code says "keep all the rows, but make the columns go in this order
# that I specified"
```
...
# use pipe to send the table.data into kable
table.data[,-1] %>%
  kable("html", caption = "Transgender Survey Participant Demographics (n=222)",
      row.names = FALSE) %>%
...
The last thing, says Nancy, is to try adding continuous variables to the table. Leslie decides to add measures of the central tendency and spread of `PHYSHLTH` since it is in the data frame. She remembers that this variable is not normally distributed, so the median and IQR are a better choice than the mean and standard deviation. Leslie uses the commands learned earlier in this chapter to re-check the median and IQR:
...
The median days of poor physical health per month in this group is 1 and the IQR shows that there is a 11 day spread that encompasses the middle half of the people in this group. Now that she knows what she is looking for in the table, Nancy tells her that adding this to the table is the same as adding a new row to a data frame using `rbind()`. However, Nancy points out that both `Variable` and `Survey participant demographics` are factor variables, and sometimes adding new rows with factors can be messy. Her suggestion is to keep all the factors as character variables instead. Then, it's easy to use `rbind()` to add a row, and convert the first two columns to a factor. She writes the code and then walks Leslie through it:

```{r c2main81}
# Almost exactly the same code as before, except some minor conversions
# so that factors remain as character strings
...
for (i in 1:(ncol(brfss.2014.small)-1)) {
  ...
  # convert Var1 from table.each into a character so it's not a factor!
  table.each$Var1 <- as.character(table.each$Var1)
  
  # Now get the frequencies using table
  # add the `stringsAsFactors` option so it's not a character!
  frequencies <- as.data.frame(table(brfss.2014.small[,i]),
  ...
  # make a new data.frame that just contains the labels
  # again, make sure stringsAsFactors = FALSE
  label.names <- data.frame(Variable = rep(c.name, times = nrow(table.each)),
                            stringsAsFactors = FALSE)
...
# ccheck the environment pane to make sure the first 2 columns are not factors
# or use the class() function

# Now, make a vector that you want to be the row. The vector needs to have the 
# same number of elements as there are columns. That is, make a complete row

physhlt.table.row <- c("PHYSHLTH",
                       "Poor health (days/mo)",
                       IQR(brfss.2014.small$PHYSHLTH, na.rm = TRUE),
                       median(brfss.2014.small$PHYSHLTH, na.rm = TRUE))

# combine this row with the data using rbind
table.data <- rbind(table.data, physhlt.table.row)

# look in the environment and check out the class of the variables
# convert them back to their original types using mutate in tidyverse

table.data <- table.data %>%
  mutate(Variable, Variable = as.factor(Variable)) %>%
  mutate(Var1, Var1 = as.factor(Var1)) %>%
  mutate(Freq, Freq = as.numeric(Freq)) %>%
  mutate(frequencies.Freq, frequencies.Freq = as.numeric(frequencies.Freq))

# as before, rename the columns and switch the order

names(table.data) <- c("Variable", "Survey participant demographics", "Percent", "Frequency")
table.data <- table.data[,c(1,2,4,3)]
...
#### Nancy's fancy code (custom functions)

The general format for making a custom function is:

```{r, eval=FALSE}
# the structure of a function (name is in Pascal case or Upper Camel Case)
NameOfFunction <- function(input arguments) {
  what the function should do
}
```

As a very basic example, lets check to see which elements in a vector of numbers is even. Maybe this is something that you might do many times in many different coding sessions for many different data sets. We can write a custom function:

```{r}
# name = isItEven
# arguments = data
# output = a logical vector for however many items was in the input data

IsItEven <- function(data) {
  # evaluate the logical statement "does the number divided by 2
  # have a remainder of 0?" and store it in an object called `dataOut`
  dataOut <- data %% 2 == 0
}

# Now you can use this for many datasets!

dataset1 <- 1:10
dataset2 <- -5:5

evens.dataset1 <- IsItEven(data = dataset1)
evens.dataset1

evens.dataset2 <- IsItEven(data = dataset2)
evens.dataset2
```

Let's apply this idea to the table we made earlier. 
