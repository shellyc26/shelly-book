...
Kiara shares one of her favorite data management tips with Leslie: create a smaller data set to work with by limiting the data to the observations that will be analyzed. There are several ways to subset a data set to only include particular observations. One method is to use the `subset()` function. Another is to simply grab the observations wanted using the square brackets `[ ]`. However, since the R-Team has been using <span style="font-family:Lucida Console, monospace;font-weight:bold">tidyverse</span>, they decide to subset via the `filter()` command. Leslie notices that the `_AGEG5YR` variable was imported with an `X` at the beginning of the variable name, so she incorporates this into the code:
...
# transgender status of MtF, FtM, or Gender non-conforming
...
This is a really tricky data management problem! Leslie asks if they can change `TRNSGNDR` to `NA` for when the `HADMAM` variable is category 9, which is the code for "Refused." This seems to Kiara like it might work, but it will require a new command. The first one that comes to Kiara from her data management work is `if_else()`. The `if_else()` code takes three arguments. The first argument is a statement that must evalue to either `TRUE` or `FALSE`; that is, the expression must be logical. In order to find out if something is equal to something else, you use two equals signs `==`. The code `3 == 9` should evaluate to `FALSE` because 3 does not equal 9. In programming, usually `!` can be read as *not*, so `!=` tests whether two things are *not* equal to each other. The code `3 != 9` should evaluate to `TRUE` because 3 and 9 are not the same number. 

The second argument of `if_else()`, is `true = `. This is where you tell R what to do if the statement from the first agrument evaluates as `TRUE`. The third argument, `false = ` is what you want to happen if the result of the statement is `FALSE`. The second and third arguments have to be the same data type. Before coding, Kiara first has Leslie say outloud what she wants the `if_else()` command to do, so that it's easier for Leslie to code. She responds with:
