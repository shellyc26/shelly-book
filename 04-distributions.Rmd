...
To follow Leslie, Kiara, and Nancy through learning about distributions and inference: 

* Download the data sets **pdmp_2018_kff_ch4.csv** and **opioid_dist_to_facility_2017_ch4.csv** from [edge.sagepub.com/harris1e](edge.sagepub.com/harris1e). 
...
* Install the following package if not already installed: <span style="font-family:Lucida Console, monospace;font-weight:bold">tidyverse</span>.
...
## Achievement 2: Understanding the characteristics and uses of a binomial distribution

The binomial distribution is a _discrete probability distribution_ used to understand what may happen when a variable has two possible outcomes. The most common example is flipping a coin, but there are many variables that have two outcomes: alive or dead, smoker or non-smoker, healthy weight or unhealthy weight, depressed or not depressed. Kiara reminds Leslie that whether a state adopted, or did not adopt, an opioid policy is another example of a variable with two possible outcomes: policy or no policy. More specifically, the _binomial distribution_ is used to represent the distribution of a _binomial random variable_, which has the following properties: 
...
* The existence of a monitoring program would be measured in each state 
...
Kiara lets Leslie know that the binomial distribution can also be displayed graphically and can be used to understand the probability of getting a specific number of successes or a range of successes (e.g., _10 or more successes_). She creates a plot of the *probability mass function* showing the distribution of probabilities of different numbers of successes. Nancy makes an example graph showing the probability of getting a certain number of states with monitoring programs in a sample when 20 are selected:
...
The probability of selecting a sample of 25 states where 15 or more states have PDMP is `r round(100*(1 - pbinom(14, size=25, prob=.63)),2)`%. Leslie does not know what to do next, in terms of drawing a sample. Luckily Nancy has some R code to help. Before they start sampling, Kiara chimes in to tell Leslie and Nancy that the `set.seed()` command is useful when conducting random sampling since it will result in the same sample to be taken each time the code is run, so it is useful for reproducibility (for more, see Box \@ref(ch4kiara). 
...
## Achievement 3: Understanding the characteristics and uses of the normal distribution {#normdistch4}

Kiara reminds Leslie that many of the variables of interest in social science are not binary so the binomial distribution and its related functions is not always useful. Instead, Kiara explains that the probability distribution for a continuous variable is the **normal distribution**. Just as the binomial distribution is determined by $n$ and $p$, the normal distribution is determined by $m$ and $s$, the mean and standard deviation of a variable. 
...
```{r message = FALSE, warning = FALSE, fig.cap = "Selecting a transformation to make the distance variable more normal for analysis."}
# transform the variable 
dist.mat.cleaned <- dist.mat %>%
  mutate(miles.cube.root = VALUE^(1/3)) %>% # Cube root transformation
  mutate(miles.log = log(VALUE)) %>% # Log transformation
  mutate(miles.inverse = 1/VALUE) %>% # Reciprocal transformation
  mutate(miles.sqrt = sqrt(VALUE)) # Square root transformation
...
## Achievement 4: Computing and interpreting z-scores to compare observations across groups {#zscore}
...
Kiara explains that this information is used to create **z-scores**, which allows description and comparison of where an observation falls compared to the other observations for a normally distributed variable. 
...
The $x$ represents the value of a variable for a single observation, $m$ is the mean of the variable, $s$ is the standard deviation of the variable. So, $z$ is the difference between the observation value and the mean value for a variable (or a *deviation score*) and is converted by the denominator into standard deviations.
...
### Using the normal distribution and samples to understand populations {#samptopop}
...
```{r warning = FALSE, message=FALSE,fig.cap="Mean miles to a substance abuse facility in 1000 samples of 500 counties each"}
...
Both of the standard error (se) values are close to the population standard deviation of .907, but are not exactly the same. The first sample standard error of 1.017 was a little above and the second sample standard error of .889 was a little below.
...
Take a sample of 200 counties and compute the mean, standard deviation, and standard error for the drug poisoning death rate in 2017. Based on the sample mean and standard error, find the range of values where the population mean drug poisoning death rate could be. Explain your results.

## Achievement 6: Computing and interpreting confidence intervals around means and proportions {#confintmean}
...
```{r echo = FALSE, warning=FALSE,message=FALSE,fig.cap = "Miles to the nearest substance abues treatment facility and 95% CI from a 2017 sample of 500 counties in the US."}
counties.500 %>% 
...
```{r echo = FALSE, message=FALSE,warning=FALSE,fig.cap = "Miles to the nearest substance abuse treatment facility and 95% CI from a 2017 sample of 500 counties in the US."}
counties.500 %>% 
  ggplot(aes(x = distance)) + 
  geom_histogram(fill = "white", color = "gray60") + 
...
```{r echo = FALSE, warning=FALSE,message=FALSE,fig.cap = "Miles to the nearest substance abuse treatment facility and 95% CI from a 2017 sample of 500 counties in the US."}
counties.500.2 %>% 
  ggplot(aes(x = distance)) + 
  geom_histogram(fill = "white", color = "gray60") + 
  geom_vline(aes(xintercept = mean(distance), color = "sampmean", linetype = "sampmean"), size = 1) +
...
samp.20.stats %>%  
  ggplot(aes(y = means, x = as.numeric(sample_num))) +
...
The American Foundation for AIDS Research also publishes the distance to needle exchange programs data. Needle exchange programs are important for preventing the spread of disease amoung intravenous drug users, including opioid users. Bring in the **opioid_dist_to_needle_exchange_2018.csv** file and examine the codebook **opioid_county_codebook.xlsx** from edge.sagepub.com/harris1e. 
