...
Leslie finds the p-value for the slope to be `<2e-16`, which is very tiny, well below .05. So, there is a tiny chance that the t-statistic for the slope would be as big as it is (or bigger) if the null hypothesis were true. The null hypothesis is therefore rejected in favor of the alternate hypothesis that the slope is statistically significantly different from zero. Leslie writes an interpretation of what she knows about the slope so far:

> The percent of uninsured residents in a county is a statistically significant predictor of the distance to the nearest syringe program (b = `r round(dist.by.unins$coefficients[2], 2)`; p = `r round(summary(dist.by.unins)$coefficients[2,4], 2)` in our sample. For every 1% increase in uninsured residents, the predicted distance to the nearest syringe program increases by 7.82 miles.
...
slopeLinePlot = dist.ssp.cleaned %>%
  ggplot(aes(x = pctunins, y = dist_SSP)) +
  geom_segment(aes(xend = pctunins, yend = predicted), color = "darkgray") +
  geom_point(aes(color = "County"), size = 2, alpha = .5) +
  geom_smooth(aes(color = "Linear\nregression\nline"),
              method = "lm",
              se = FALSE) +
  theme_classic(base_size = 10) +
  labs(y = "Miles to syringe program", x = "Percent uninsured") +
  scale_color_manual(values = c("#7463AC", "#78A678", "deeppink"),
                     name = "")

meanLinePlot = dist.ssp.cleaned %>%
  ggplot(aes(x = pctunins, y = dist_SSP)) +
  geom_segment(aes(xend = pctunins, yend = mean(dist_SSP)), color = "black",
               linetype = "dotted") +
  geom_point(aes(color = "County"), size = 2, alpha = .5) +
  geom_hline(aes(yintercept = mean(dist_SSP), color = "Mean\ndistance\nto SSP")) +
  theme_classic(base_size = 10) +
  labs(y = "Miles to syringe program", x = "Percent uninsured") +
  scale_color_manual(values = c("#7463AC", "deeppink"),
                     name = "")

gridExtra::grid.arrange(slopeLinePlot, meanLinePlot, nrow = 2)

```

Kiara admits that it's a little difficult to see for counties with a low percent of uninsured individuals (the left portion of each plot), but she points out to Leslie that on the right side, the dark gray bars between the points and the linear regression line (green line in top plot) are shorter than the dotted black lines between the points and mean line (bright pink in bottom plot). This is essentially the hypothesis being tested by the F-test and corresponding F-statistic.
...
$$
\begin{equation}
F = \frac{\frac{\sum\limits_{i=1}^n({\hat{y_i}-\bar{y}})^2}{k-1}}{\frac{\sum\limits_{i=1}^n({y_i - \hat{y_i}})^2}{n-k}}
 (\#eq:fforlinear)
\end{equation}
$$
...
* $\bar{y}$ is the mean of the observed outcomes of distance to syringe program

Kiara explains that the numerator of $F$ is essentially how much the predicted values differ from the mean observed value, _on average_. This is divided by how much the predicted values differ from the actual observed values, _on average_. Kiara repeats that the $F$-statistic is how much a predicted value differs from the mean value on average---which is explained variance, or how much better (or worse) the prediction is than the mean at explaining the outcome---divided by how much an observed value differs from the predicted value on average, which is the residual information or unexplained variance. Sometimes these are referred to in similar terminology that is used in ANOVA: the numerator is the $MS_{regression}$ (where $MS$ stands for "mean square") divided by the $MS_{residual}$, referring to everything *not* explained by the regression or the variance that is left over.  

Nancy notes that the $F$-statistic is always positive due to the squaring of the terms in the numerator and denominator. As a result, the $F$ distribution starts at zero (when the regression line is exactly the same as the mean) and goes to the right. The shape of the $F$ distribution depends on the the number of parameters in the statistical model and the sample size. Nancy finds a tutorial [@RPubsftest] with instructions for plotting examples of the $F$-distribution and she graphs a few different versions with different numbers of parameters and sample sizes. She explains that the $F$-statistic is usually written with two degrees of freedom ($df$) numbers. The first number in the parentheses after the $F$ is the degrees of freedom for the numerator. This value is one less than the number of parameters, often written `k-1`. The second number in the parentheses after an F-statistic is the degrees of freedom for the denominator, or `n-k`. 

```{r fstatsfig, echo=FALSE, fig.cap = "Examples of the distribution of probability density for F."}


ggplot(data.frame(x=c(0,5)), aes(x=x)) +
  stat_function(fun=match.fun(df), args=list(df1=1, df2=20), aes(color="F(1, 20)"), size = 1) +
  ...
  scale_color_manual(values = c("F(1, 20)" = "#78A678",
                                "F(2, 50)" = "#7463AC",
                                "F(5, 100)" = "deeppink",
                                "F(10, 200)" = "dodgerblue2"),
                     name = "Distribution",
                     breaks = c("F(1, 20)", "F(2, 50)", "F(5, 100)",
                                "F(10, 200)")) +
  labs(x = "F-statistic", y = "Probability density") +
  theme_minimal() 
```

The more the model explains the variation in the outcome, the larger the $F$-statistic gets. Like $t$-statistics and $\chi^2$ statistics, larger values of $F$-statistics are less likely to occur when there is no relationship. Leslie asks about graphing the $F$-distribution from the linear regression model for distance to syringe program. Nancy looks back at the ouput and sees there are two parameters, slope and intercept. The numerator degrees of freedom are $k-1$, and since there are two parameters ($k$ = 2), then the numerator degrees of freedom are $2-1 = 1$. For the denominator, the sample size is used to determine the denominator degrees of freedom ($n-k$). Here, the denominator degrees of freedom are $500 - 2 = 498$. Leslie verifies that these degrees of freedom $F$(1,498) align with the output of the linear model `dist.by.unins`. Nancy uses the code from above and plots the $F$-distribution for the model.
...
ggplot(data.frame(x=c(0,5)), aes(x=x)) +
  ...
  geom_vline(aes(xintercept = summary(dist.by.unins)$fstatistic[1], color = "F(1, 498) = 102.2"), show.legend = FALSE, size = 1) +
  labs(x = "F-statistic", y = "Probability density") +
  scale_color_manual(values = c("F(1, 498) distribution" = "#7463AC",
                                "F(1, 498) = 102.2" = "deeppink"),
                     name = "",
                     breaks = c("F(1, 498) distribution", "F(1, 498) = 102.2")) +
  scale_x_continuous(limits = c(0,110), breaks = seq(from = 0,
                                                     to = 110,
                                                     by = 10)) + 
  theme_minimal() 
```

Just like with the $t$-statistic and $\chi^2$, the probability of an $F$-statistic this large or larger *if the null is true* is the area under the $\color{#7463AC}{\text{purple}}$ curve starting at the $\color{#FF1493}{\text{pink}}$ line and going right or `r round(summary(dist.by.unins)$fstatistic[1], 2)`. There is really very little space under the curve from the value of $F$ to the right, which is consistent with the tiny $p$-value ($p$ < .001 per bottom of the `summary(dist.by.unins)` output). Leslie interprets this as nearly a 0% ($p$ < .001) chance that an $F$-statistic this large would occur under the null hypothesis that there is no relationship between percent uninsured and distance to syringe program. Essentially, the $F$-statistic and the associated $p$-value suggest a statistically significant association between percent uninsured and distance to syringe program at the county level.
...
H0: A model including uninsurance percent is no better at explaining the distance to syringe programs for people in a county than a baseline model, where the baseline is simply the mean of the distance to syringe programs. 

H0: A model including uninsurance percent is better at explaining the distance to syringe programs for people in a county than a baseline model, where baseline is simply the mean of the distance to syringe programs.
...
The $R^2$ is the amount of variation in the outcome that the model explains and is reported as a measure of **model fit**. For the relationship between uninsured percent and distance to syringe program the $R^2$ is `r round(summary(dist.by.unins)$r.squared, 2)`. To get the percent of variance explained by the model, multiply by 100 for `r round(100*summary(dist.by.unins)$r.squared, 2)`% of the variation in distance to syringe programs is explained by the percentage of uninsured people living in a county. Kiara explains that simply subtracting one from $R^2$ ($1 - R^2$) and again multiplying by 100 for a percent will give the percent of variance *not* explained by the model (here, `r round(100*(1-summary(dist.by.unins)$r.squared), 2)`). 

Leslie sees another $R^2$ labeled **Adjusted R-squared**---or ${R}^2_{adj}$---in the output. Kiara explains that the value of $R^2$ tends to increase with each additional variable added to the model, whether the variable actually improves the prediction or explanation of the outcome or not. The ${R}^2_{adj}$ penalizes the value of ${R}^2$ a small amount for each additional variable added to the model to ensure that the ${R}^2_{adj}$ only increases when the additional predictors in a model explain a notable amount of the variation in the outcome. Essentially, Kiara explains, the ${R}^2_{adj}$ keeps analysts from just adding all the variables they have in order to get the highest possible value for model fit. The ${R}^2_{adj}$ is more commonly reported than the $R^2$.
...
- the significance of the slope ($t$ and $p$; confidence intervals if available)  
...
> A simple linear regression analysis found that the percent of uninsured residents in a county is a statistically significant predictor of the distance to the nearest syringe program ($b$ = `r round(dist.by.unins$coefficients[2], 2)`; $p$ < .001. For every 1% increase in uninsured residents, the expected distance to the nearest syringe program increases by 7.82 miles. The value of the slope is likely between 6.30 and 9.34 in the population that the sample came from (95% CI: 6.30-9.34). So, with every 1% increase in uninsured residents, there is likely a 6.30 to 9.34 increase in the miles to the nearest syringe program. The model was statistically significantly better than the baseline model (here, the mean of the distance to syringe variable) at explaining distance to syringe program ($F$(1, 498)=102.2; $p$ < .001) and explained 16.9% of the variance in the outcome. These results suggest that communities with lower insurance rates are further from this resources, which may exacerbate existing health disparities.
...
### The seven assumptions of simple linear regression
...
* The variance of the residuals is constant with the points distributed equally around the line (homoscedasticity) 
* The residuals are independent 
* The residuals are normally distributed 

### Checking the independent observations assumption 

The R-Team talks about this for a few minutes. While it seems like the data have been collected in a reasonable way, one could argue that counties in the same state are not really independent because, like siblings in the same household, they may have many of the same experiences and characteristics and therefore may not vary as much from each other as truly independent areas would. This assumption is often seen as a possible problem for geographic data, Kiara comments, but researchers continue to use linear regression and other similar models to work this type of data. They decide that this is a definite limitation and move on to the next assumption.
...
```{r distsyringe, message=FALSE, warning=FALSE,fig.cap = "Distribution of distance to syringe program in 500 counties in 2018 (data source: amFAR)."}
...
```{r distunins, message=FALSE, warning=FALSE,fig.cap = "Distribution of percent uninsured in 500 counties in 2017 (data source: amFAR)."}
...
The *linearity* assumption is met if a scatterplot of the two variables shows a relationship that falls along a line. The earlier plot showing purple points and a green straight line drawn through the data suggests that this assumption may be met. When graphed, the points fall generally the straight line. However, unlike the graph of water access and and female education in the previous chapter, the data points seem to not follow the line as well. Using a _Loess curve_, we can see the relationship between the two variables without constraining the line to be straight. In this case, a $\color{#FF1493}{\text{pink}}$ Loess curve shows deviation from a linear relationship. 

```{r loessreg, warning = FALSE, message = FALSE,fig.cap = "Relationship between percent without health insurance 2016 and distance to needle exchange 2018 in 500 counties (data source: amFAR)."}
# percent without health insurance and distance to needle exchange
dist.ssp.cleaned %>%
  ggplot(aes(x = pctunins, y = dist_SSP)) +
  geom_point(aes(color = "County"), size = 2) + 
...
Kiara explains that both of the last three assumptions are about residuals and reminds Leslie that residuals are the distances between each data point and the regression line. Conceptually, residuals are the variation in the data that the regression line does not explain.

Leslie recognizes homoscedasticity from the correlation chapter. Kiara explains that the idea is still the same such that the points around the line should be roughly evenly distributed. But in regression, the points are the residuals. That is, the homoscedasticity assumption in regression is that the variability of the residuals is constant across all levels of the independent variable; another way of saying this is that there should be constant error variance. If they are not constant (for example, maybe a model has very small residuals for small values of x, but very large residuals at larger values of x), this would be referred to as heteroscedasticity. One way to check for this is to make a scatter plot of the model-predicted values (sometimes known as "fitted" values) against the residual values. If homoscedasticity is present, then the points of the scatter plot should look like a random cloud of points with no clear trend. The main thing to watch out for is any trend that makes it seem that the points are organized in a non-random fashion. For instance, a common form would be the points make a cone shape or have a fan effect. Nancy makes a quick plot to test the constant error variance assumption:

```{r echo = FALSE}
# dataframe of just residuals and predicted
homoscedasticity.data = data.frame(Fitted = dist.by.unins$fitted.values,
                                   Residuals = dist.by.unins$residuals)

ggplot(data = homoscedasticity.data, aes(x = Fitted, y = Residuals)) +
  geom_point(color = "#7463AC") +
...
Leslie recalls that a flat line on a scatter plot indicates that there is no relationship between the two variables. In this plot, a black dashed line is shown to indicate that there *should* be no relationship between the fitted (or predicted) values and the residuals. This line is a helpful reference point for looking at these types of graphs.

For the homoscedasticity assumption to be met, the points should be roughly evenly distributed around the black dashed line with no clear patterns. It should look like a cloud `r emo::ji("cloud")` or like you just randomly put points on the graph. Yet in this plot, we do see a clear pattern. It looks like a fan or a cone such that the points on the left side are much closer to the idealized "no relationship" line than the points on the right side. This indicates that residuals are heteroscedastic, and thus the homoscedasticity assumption is not met.

Kiara reminds Leslie that the Breusch-Pagan can be used to formally test the null hypothesis that the variance of the errors (or residuals) is constant. Nancy shows them the `bptest()` function from the <span style="font-family:Lucida Console, monospace;font-weight:bold">lmtest</span> package:
...
The Breusch-Pagan test statistic has a tiny $p$-value associated with it ($BP$ = `r round(const.var.test$statistic, 2)`; $p$ = `r round(const.var.test$p.value, 2)`), indicating that the null hypothesis that the variance is constant would be rejected. Leslie concludes that the assumption of constant variance is not met, which converges with what the plot of fitted values vs. residuals showed. 

### Testing the independence of residuals assumption 

The next assumption for residuals is that the residuals are independent or unrelated to each other. Residuals that are independent are residuals that do not follow a pattern. Kiara explains that a pattern in the residuals suggests that the regression model is doing better for certain types of observations and worse for others. 
...
```{r waring = FALSE, message = FALSE,fig.cap="Distribution of residuals for model of uninsured percent and distance to syringe program for 500 counties (data source: amFAR)"}
...
```{r warning = FALSE, message = FALSE, fig.cap="Distribution of residuals for model of uninsured percent and distance to syringe program for 500 counties (data source: amFAR)"}
# check residual plot of uninsured percent and distance to syringe program
data.frame(dist.by.unins$residuals) %>%
...
Leslie notes that the linear regression analysis meets some and fails some of the assumptions. The assumptions it meets are: continuous variables and independence of residuals. The assumptions it fails are: normally distributed variables, linearity, homoscedasticity, and normally distributed residuals. The team also thinks it may violate independence of observations, but this is not as clear. Because it does not meet all the assumptions, Leslie knows that the model is considered biased and should be interpreted with caution. Specifically, the results of a biased model are not usually considered generalizable to other observations outside the sample. 
...
Kiara notes that uninsured percent accounted for `r round(100*summary(dist.by.unins)$r.squared, 1)`% of the variation in distance to syringe program for counties, leaving about `r round(100*(1-summary(dist.by.unins)$r.squared), 1)`% still explained. Kiara thinks it is time to add in additional variables to see if they account for some of the variation in distance to syringe program. She thinks that bigger cities are more likely to have these programs, so the `metro` variable seems like it might help to explain how far away a county is from a syringe program. Certainly Figure \@ref(fig:boxmetro) and Figure \@ref(fig:violinmetro) indicated that the distance to syringe programs was further in non-metro areas.
...
Leslie sees now that the two slopes are the same, but that the $y$-intercept is lower for the `metro` group compared to the `non-metro` group. Because the slope is the same, this means that, given a certain uninsurance percent, we would expect that a resident of a metro county will have 28.05 fewer miles to drive to a syringe program. So, a person in a metro county with 20% uninsured is predicted to have to drive 28.05 fewer miles to the syringe exchange compared to a person in a non-metro county with 20% uninsured.
...
```{r warning = FALSE, message = FALSE,fig.cap = "Distribution of transformed distance to syringe program in 2018 for 500 counties (data source: amFAR)"}
...
```{r warning = FALSE, message = FALSE, fig.cap = "Distribution of transformed HIV prevalence in 2018 for 500 counties (data source: amFAR)"}
...
If the absolute value of the correlation coefficient were .7 or higher, this would indicate a strong relationship with a large amount of shared variance between the two variables and therefore a problem with multicollinearity.
...
The result will be `1` if there is no shared variance at all. If there is any shared variance, the VIF will be greater than one. If the VIF is large, this indicates that `pctunins` shares a lot of variance with the `metro` and `log(HIVprevalence)` variables. A VIF of 2.5, for example, would indicate that the $R^2$ was .60 and so 60% of the variation in `pctunins` was explained by `metro` and `log(HIVprevalence)`. While there is no consistent cutoff recommended for the size of a VIF that indicates too much multicollinearity, 2.5 is often used. 
...
```{r warning = FALSE, message = FALSE,fig.cap = "Relationship between the log of HIV prevalence in 2017 and transformed distance to needle exchange 2018 in 500 counties (data source: amFAR)."}
...
```{r warning = FALSE, message = FALSE, fig.cap = "Relationship between percent uninsured in 2017 and transformed distance to needle exchange 2018 in 500 counties (data source: amFAR)."}
...
Leslie also remembers that plotting the fitted values vs. the residuals is another key way of examining this constant error variance assumption:

```{r}
homoscedasticity.data.full = data.frame(Fitted = dist.full.model$fitted.values,
                                   Residuals = dist.full.model$residuals)

ggplot(data = homoscedasticity.data.full, aes(x = Fitted, y = Residuals)) +
  geom_point(color = "#7463AC") +
  geom_smooth(method = "lm", se = FALSE, color = "black",
              linetype = "dashed") +
  theme_minimal() +
...
While the points look more cloud-like on the left side of the graph, the fan effect is still there on the right side of the graph. This converges with the Breusch-Pagan test.
...
```{r warning = FALSE, message = FALSE,fig.cap="Distribution of residuals for model explaining distance to syringe program for 500 counties (data source: amFAR)"}
# check residual plot of uninsured percent and distance to syringe program
...
The $R^2$, is interpreted as the percentage of variance in the outcome that is explained by the model. The ${R}^2_{adj}$ penalizes the $R^2$ for each variable in the model, resulting in a lower value that is considered a more accurate reflection of how well the model explains the variability in the outcome. 

Overall model significance is determined using the $F$-statistic, which is a ratio of explained variance to unexplained variance. When the $F$-statistic is large, that indicates there is more explained variance relative to unexplained and the model is likely reflecting a true relationship. 

#### Achievement 6 recap: Checking assumptions and conducting diagnostics  

Statistical tests rely on underlying assumptions about the characteristics of the data. When these assumptions are not met, the results may not reflect the true relationships among the variables. The variable type can be checked by examining the two variables to be sure they are continuous. Histograms and QQ-Plots can be used to determine if the variables are normally distributed. A scatterplot with a Loess curve is useful for examining linearity. A scatterplot and Breusch-Pagan test can aid in identifying problems with constant error variance. Residual independence is tested using the Durbin-Watson statistic and residual normality can be examined with the histogram and QQ-plot. Outliers and influential values can be identified using standardized residuals, df-betas, Cook's D, and Leverage values. Observations that are identified as problematic by more than one measure should be examined and, if the data appear unreliable for the observation, the observation can be fixed or dropped before re-estimating and testing the model.
